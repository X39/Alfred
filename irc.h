#pragma once
	...
	#undef FNC
#endif
#define FNC(txt) irc_client_##txt
...
#define RPL_MYINFO 004				//"<servername> <version> <available user modes> <available channel modes>" - The server sends Replies 001 to 004 to a user upon successful registration.
#define RPL_BOUNCE 005				//"Try server <server name>, port <port number>" - Sent by the server to a user to suggest an alternative server.  This is often used when the connection is refused because the server is already full.
#define RPL_USERHOST 302			//":*1<reply> *( " " <reply> )" - Reply format used by USERHOST to list replies to the query list.  The reply string is composed as follows: reply = nickname [ "*" ] "=" ( "+" / "-" ) hostname The '*' indicates whether the client has registered as an Operator.  The '-' or '+' characters represent whether the client has set an AWAY message or not respectively.
#define RPL_ISON 303				//":*1<nick> *( " " <nick> )" - Reply format used by ISON to list replies to the query list.
...
#define RPL_LISTEND 323				//":End of LIST" - Replies RPL_LIST, RPL_LISTEND mark the actual replies with data and end of the server's response to a LIST command.  If there are no channels available to return, only the end reply MUST be sent.
...
#define RPL_ENDOFLINKS 365			//"<mask> :End of LINKS list" - In replying to the LINKS message, a server MUST send replies back using the RPL_LINKS numeric and mark the end of the list using an RPL_ENDOFLINKS reply.
...
#define RPL_YOURESERVICE 383		//"You are service <servicename>" - Sent by the server to a service upon successful registration.
...
#define RPL_SERVLISTEND 235			//"<mask> <type> :End of service listing" - When listing services in reply to a SERVLIST message, a server is required to send the list back using the RPL_SERVLIST and RPL_SERVLISTEND messages.  A separate RPL_SERVLIST is sent for each service.  After the services have been listed (or if none present) a RPL_SERVLISTEND MUST be sent.
...
#define RPL_ADMINLOC 257			//":<admin info>"
#define RPL_ADMINLOC2 258			//":<admin info>"
#define RPL_ADMINEMAIL 259			//":<admin info>" - When replying to an ADMIN message, a server is expected to use replies RPL_ADMINME through to RPL_ADMINEMAIL and provide a text message with each.  For RPL_ADMINLOC1 a description of what city, state and country the server is in is expected, followed by details of the institution (RPL_ADMINLOC2) and finally the administrative contact for the server (an email address here is REQUIRED) in RPL_ADMINEMAIL.
#define RPL_TRYAGAIN 263			//"<command> :Please wait a while and try again." - When a server drops a command without processing it, it MUST use the reply RPL_TRYAGAIN to inform the originating client. Error replies are found in the range from 400 to 599.
...
#define ERR_TOOMANYTARGETS 407		//"<target> :<error code> recipients. <abort message>" - Returned to a client which is attempting to send a PRIVMSG/NOTICE using the user@host destination format and for a user@host which has several occurrences. - Returned to a client which trying to send a PRIVMSG/NOTICE to too many recipients. - Returned to a client which is attempting to JOIN a safe channel using the shortname when there are more than one such channel.
...
#define ERR_BADMASK 415				//"<mask> :Bad Server/host mask" - 412 - 415 are returned by PRIVMSG to indicate that the message wasn't delivered for some reason. ERR_NOTOPLEVEL and ERR_WILDTOPLEVEL are errors that are returned when an invalid use of "PRIVMSG $<server>" or "PRIVMSG #<host>" is attempted.
...
#define IRC_PASS 1000
/*
	PASS <password>

	ERR_NEEDMOREPARAMS              ERR_ALREADYREGISTRED
*/
#define IRC_NICK 1001
/*
	NICK <nickname>
...
*/
#define IRC_USER 1002
/*
	USER <user> <mode> <unused> :<realname>

	ERR_NEEDMOREPARAMS              ERR_ALREADYREGISTRED
*/
#define IRC_OPER 1003
/*
	OPER <name> <password>
...
*/
#define IRC_QUIT 1006
/*
QUIT [ :<quit message> ]
*/
#define IRC_JOIN 1007
/*
	JOIN <channel>
